...
import { getProviderKeyForModel } from "@/lib/ai/user-keys";
import type { Vote } from "@/lib/db/schema";
import { extractSandboxEventsFromParts } from "@/lib/e2b/sandbox-events";
...
import { SandboxBrowser } from "./sandbox-browser";
...
export function Chat({
  ...
  const {
    ...
    transport: new DefaultChatTransport({
      ...
      prepareSendMessagesRequest(request) {
        ...
        const userApiKey = getProviderKeyForModel(currentModelIdRef.current);
        if (userApiKey) {
          headers.set("x-user-provider-api-key", userApiKey);
        }
  ...
  useEffect(() => {
    ...
    if (
      latestEvent.status === "terminated" ||
      latestEvent.status === "already_not_found"
    ) {
      window.localStorage.removeItem(`sandbox:${id}`);
      setSandboxId(null);
      return;
    }

    if (latestEvent.sandboxId) {
      setSandboxId(latestEvent.sandboxId);
      window.localStorage.setItem(`sandbox:${id}`, latestEvent.sandboxId);
    }
  }, [id, messages]);
  ...
  useEffect(() => {
    ...
    const stored = window.localStorage.getItem(`sandbox:${id}`);
    if (stored) {
      setSandboxId(stored);
    }
  }, [id, sandboxId]);
