...
import { createTonSandboxTools } from "@/lib/ai/tools/ton-sandbox";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const userProviderApiKey =
      request.headers.get("x-user-provider-api-key")?.trim() || undefined;
    ...
    const stream = createUIMessageStream({
      ...
      execute: async ({ writer: dataStream }) => {
        ...
        const e2bStatus = getE2BSandboxStatus();
        if (userType !== "guest" && !e2bStatus.enabled) {
          logE2BConfigWarningOnce();
        }

        const allowSandboxTools = userType !== "guest" && e2bStatus.enabled;
        const result = streamText({
          model: getLanguageModel(selectedChatModel, {
            apiKey: userProviderApiKey,
          }),
          system: systemPrompt({
            selectedChatModel,
            requestHints,
          ...
          activeTools: isReasoningModel
            ...
            : ([
                ...baseActiveTools,
                ...tonToolNames,
                ...(allowSandboxTools ? tonSandboxToolNames : []),
              ] as Array<
                | (typeof baseActiveTools)[number]
                | keyof typeof tonTools
          ...
          tools: {
            ...
            ...tonTools,
            ...(allowSandboxTools ? tonSandboxTools : {}),
          },
...
export async function DELETE(request: Request) {
  ...
  if (getE2BSandboxStatus().enabled) {
    try {
        ...
        await Promise.allSettled(
          sandboxIds.map(async (sandboxId) => {
            try {
              ...
              console.warn(
                "[E2B] Failed to terminate sandbox during chat deletion",
                {
                  chatId: id,
                  sandboxId,
                  error,
      ...
      console.warn("[E2B] Sandbox cleanup skipped due to pre-delete error", {
        chatId: id,
        error,
      });
    }
